...
## Remove 50% of MSM isolates 
subsample_MSM = read.csv('1_Data/subsample/line_subsampled_listofisolatesforphylowave.csv')
subsample_MSM = subsample_MSM[,2]
tmp = metadata[which(metadata$Group == 'pMSM'),]
to_remove = tmp$Accession[is.na(match(tmp$Accession, subsample_MSM))]
metadata = metadata[which(is.na(match(metadata$Accession, to_remove))),]
...
## Save results
########################################################################################################################################
save.image(file='2_analysis_index/3_sensitivity_subsample/Initial_index_computation_and_parameters_subsampled_20260313.Rdata')
########################################################################################################################################
